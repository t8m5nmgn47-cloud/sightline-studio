...
// display-format a US phone: +13035005783 → (303) 500-5783
const fmtPhone = ph => { const d=(ph||'').replace(/[^0-9]/g,'').replace(/^1(?=\d{10}$)/,'');
...
function derivePalette(colors){
  ...
  // capture ranks colours by how heavily the site actually uses them — respect
  // that order. The FIRST sufficiently-vivid colour is the brand; sorting by
  // raw saturation let one stray framework/utility colour outrank the real brand.
