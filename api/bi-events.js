...
const EVENT_TYPES = new Set([
  "campaign_sent", "campaign_result", "post_published", "promotion_started",
  "email_opened", "link_clicked", "lead_created", "booking_created", "conversion",
  "sale_completed", "review_received", "experiment_planned", "experiment_running", "experiment_completed",
]);
...
function normalizeEvent(input = {}) {
  ...
  return {
    entity_key: entityKey, event_type: eventType, occurred_at: occurred.toISOString(),
    channel: clean(input.channel, 80) || null, campaign_id: clean(input.campaign_id, 160) || null,
    offer_id: clean(input.offer_id, 160) || null, creative_id: clean(input.creative_id, 160) || null,
    customer_ref: clean(input.customer_ref, 160) || null,
    local_weekday: DAY_NAMES.includes(storedDay) ? storedDay : null, local_hour: localHour,
    value_numeric: input.value_numeric == null || input.value_numeric === "" ? null : Number(input.value_numeric), metadata,
  };
}
...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      const rows = await sbSelect("bi_events", `select=id,event_type,occurred_at,channel,campaign_id,offer_id,creative_id,local_weekday,local_hour,value_numeric,metadata&entity_key=eq.${encodeURIComponent(domain)}&order=occurred_at.desc&limit=500`);
  ...
  if (req.method === "POST") {
    try {
      ...
      for (const row of rows) if (row.value_numeric != null && !Number.isFinite(row.value_numeric)) throw new Error("value_numeric must be a number.");
